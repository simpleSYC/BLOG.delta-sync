var gitblog = function (config) {
  ...
  self.set = function () {
    ...
    if (self.options.code != null && self.options.redirect_url != null) {
      window.location.href =
        config.server_link + "?code=" + self.options.code + "&redirect_url=" + self.options.redirect_url + "&client_id=" + config.client_id + "&client_secret=" + config.client_secret;
    }
  };
  ...
  self.utc2localTime = function (time) {
    let svg_time = "  <i class='far fa-clock'></i> ";
    ...
    return `<i class='far fa-calendar-alt'></i>  ` + date + " / " + month + " / " + year + svg_time + hour + ":" + minute + ":" + second;
  };
    ...
    init: function () {
      ...
      if (icon.hidden_img != undefined && icon.hidden_img != null) {
        document.getElementById("div_" + icon.name).innerHTML +=
          ...
          '" style="width: ' +
          icon.width +
          "px; position: absolute; left: calc(50% - " +
          this.position +
          'px); bottom: 180px; transition: all 0.3s ease 0s; box-shadow: rgb(149, 165, 166) 0px 0px 5px; transform: translateY(-20px); z-index: -1; opacity: 0">';
  ...
  Pages.prototype = {
    getNum: function (request_url) {
      ...
      $.ajax({
        type: "get",
        headers: {
          Authorization: "36e5e56d88e012d48d2b:abd0316ba142ec85280d0cf07365e26b3957d318",
        },
        url: request_url,
  ...
  Reaction.prototype = {
    ...
    show: function (type, id) {
      ...
      if (reaction.isLike == false) {
        document.getElementById(id).innerHTML = '<img src="./GLOBAL/MEDIA/heart.svg" style="height:20px;float:left">';
  ...
  Comment.prototype = {
    ...
    addComment: function (data) {
      ...
      document.getElementById("comment-list").innerHTML +=
        ...
        '<div style="float:right;cursor:pointer" id="' +
        data.id +
        '"></div>' +
        '</div><div class="gitment-comment-body gitment-markdown">' +
        data.body_html +
        "</div></div>";
      ...
      if (this.login == true) {
        $.ajax({
          ...
          success: function (data) {
            ...
            avatar.innerHTML =
              '<a class="gitment-comment-avatar" href=' +
              window.localStorage.user_url +
              ' target="_blank">' +
              '<img class="gitment-comment-avatar-img" src=' +
              window.localStorage.user_avatar_url +
              "></a>";
        ...
        avatar.innerHTML =
          '<a class="gitment-editor-avatar" id="gitment-avatar" title="login with GitHub">' +
          '<img src="./GLOBAL/MEDIA/gitment-github-icon.svg" class="gitment-github-icon" style="width:44px">' +
          "</a></div>";
    ...
    addItem: function (data) {
      ...
      for (var i in data) {
        ...
        document.getElementById("issue-list").innerHTML +=
          ...
          '">' +
          data[i].title +
          '</a></h4><div class="excerpt"><p class="issue">' +
          data[i].body +
          "</p></div>" +
          '<ul class="meta"><li>' +
          data[i].user.login +
      ...
      $.ajax({
        ...
        success: function (data) {
          if (self.options.q == undefined || self.options.q == null) {
            if (data.length == 0) {
              document.getElementById("issue-list").innerHTML = "There isnt any contet relaterd to this  # TAG ..";
            } else {
              issue.addItem(data);
            }
          } else {
            ...
            var newHtml;
            console.log(self.options.q);
            if (self.options.q != "") newHtml = html.replaceAll(self.options.q, `<font style="background-color:yellow;">` + self.options.q + "</font>");
            else newHtml = html;
  ...
  Buttons.prototype = {
    ...
    init: function () {
      ...
      this.getByClass(document.getElementsByTagName("body")[0], "main")[0].addEventListener(
        "mousedown",
        function () {
          if ($(".main").css("transform") != "matrix(1, 0, 0, 1, 0, 0)") {
